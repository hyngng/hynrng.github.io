...
그래서 업데이트 방법을 찾아보고 있었는데, 찾다보니 하나 알게 된 것이 제가 블로그를 처음 개설할 때 **Chirpy starter** 방식을 사용했다는 것이었습니다. 이 방식은 과정은 간단하지만 커스터마이징 폭이 상당히 좁다고 합니다.  
...
다만 이렇게 사용해보니 prompt 또한 `<p>` 태그를 이용하는 관계로 상단에 공백을 같이 생성하는 문제가 있어, prompt가 이용하는 `<blockquote>` 태그에 한해 공백이 생성되지 않도록 별도로 코드를 추가해 주었습니다.
