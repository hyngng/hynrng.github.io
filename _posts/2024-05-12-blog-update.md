...
마침 요즘 블로그의 여러 기능을 하나하나 써보는 재미에 맛들렸는데 때마침 이런 신기능을 넣어주면 궁금해서 참을 수가 없습니다. 이전까지는 큰 변화점이 없어서 업데이트를 미루고 있었는데, 이번 기회에 바로 업데이트에 도전해주었습니다.
...
병합이 끝나면 CSS와 자바스크립트 파일 컴파일을 위해 위 명령어를 입력해줍니다.
