...
게임을 개발하다 보니 개념적 정리가 필요한 기술 사항들이 많다는 것을 점점 느끼고 있습니다. 평소 메모를 위해 노션이나 옵시디언을 활용하고는 있지만 아무래도 저 혼자 글을 작성하면 너무 편하게 쓰는 나머지 금세 알아보기 어려운 구조가 되어버리더라구요. 그래서 앞으로 따로 숙지가 필요한 것들은 한 번씩 심도있게 짚고 넘어갈 겸 블로그에 차근차근 하나씩 정리해두려고 합니다.
...
싱글톤 패턴을 가장 먼저 소개하는 이유는 이것이 직관적이기 때문입니다. 게임 개발을 배울 때 코드를 작성하는 법이 어느정도 손에 익었다 싶으면 이 패턴을 접하게 되는 분들이 많으시죠. 싱글톤 패턴은 그만큼 개념도 간단하고, 사용하기도 어렵지 않습니다.
...
싱글톤 스크립트의 인스턴스는 기본적으로 단 한 개로 유지되므로 개념적으로는 싱글톤 인스턴스를 여러 개 사용할 수 없습니다. 코드를 복사해 어찌저찌 사용할 수는 있지만 비효율적이죠. 대신, 여러 개의 서로 다른 싱글톤 인스턴스가 필요한 경우 제네릭을 사용하여 `Singleton<T>`의 형태로 서로 다른 싱글톤 스크립트를 여러개 생성하여 사용할 수 있습니다.
...
다만 싱글톤 패턴은 논란이 꽤 있는 프로그래밍 패턴입니다. 주요 요지는 편리하기 때문에 남용하기 쉽다는 것이죠. 싱글톤 스크립트가 너무 많은 역할이나 데이터를 담당하게 되면 싱글톤 인스턴스와 다른 클래스의 결합이 강해지며, 코드가 지저분해지기 때문에 싱글톤 인스턴스의 변경 주체나 시점을 알기 어려워질 수 있다는 겁니다.
...
그래도 여전히 사용하기 쉽고, `GetComponent()`나 `Find()` 등 무거운 함수의 중복 실행을 피할 수 있다는 점 때문에 프로젝트 규모에 따라 충분히 사용할만한 가치가 있는 프로그래밍 패턴입니다.