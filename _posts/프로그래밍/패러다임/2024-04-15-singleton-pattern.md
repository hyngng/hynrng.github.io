...
게임을 개발하다 보니 개념적 정리가 필요한 기술 사항들이 많다는 것을 점점 느끼고 있습니다. 평소 메모를 위해 노션이나 옵시디언을 활용하고는 있지만 아무래도 저 혼자 글을 작성하면 너무 편하게 쓰는 나머지 금세 난잡해지더라구요.

그럼에도 원래는 개인 고집으로 블로그 분위기가 너무 딱딱해지지 않았으면 해서 기술적 사항에 대한 포스트는 일부로 자세히 작성하지 않으려 했는데, 최근 블로그의 어원이 Web + Log라는 것을 듣고 단순한 공부 관련 포스트도 "남길 만한 글"로서는 나름 괜찮겠다는 생각이 들었습니다. 그래서 앞으로 따로 기록이 필요한 것들은 아예 한 번씩 심도있게 짚고 넘어갈 겸 블로그에 차근차근 하나씩 정리해두려고 해요.
...
기존까지는 싱글톤 인스턴스가 없을 때 인스턴스를 단순히 `null`로 처리하고 새 인스턴스를 생성하지 않으므로, 외부 스크립트에서 인스턴스에 접근할 때 `Instance`가 유무를 체크해야 하는 번거로움이 있었지만 이것이 불편할 경우 프로퍼티 부분을 수정해 위와 같이 인스턴스화 코드까지 연결할 수 있습니다.
