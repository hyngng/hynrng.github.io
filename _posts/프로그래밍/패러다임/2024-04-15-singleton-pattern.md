...
다만 싱글톤 패턴은 논란이 꽤 있습니다. 주요 요지는 남용하기 쉽다는 것이죠. 싱글톤이 너무 편리한 나머지 스크립트가 너무 많은 역할이나 데이터를 담당하게 되면 싱글톤 인스턴스와 다른 클래스의 결합이 강해지며, 코드가 지저분해지기 때문에 싱글톤 인스턴스의 변경 주체나 시점을 알기 어려워질 수 있다는 겁니다.
