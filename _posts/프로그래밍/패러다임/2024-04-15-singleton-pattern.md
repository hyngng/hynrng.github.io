...
다만 싱글톤 패턴은 논란이 꽤 있는 패턴입니다. 주요 요지는 남용하기 쉽다는 것이죠. 싱글톤 스크립트가 너무 많은 역할이나 데이터를 담당하게 되면 싱글톤 인스턴스와 다른 클래스의 결합이 강해지며, 코드가 지저분해지기 때문에 싱글톤 인스턴스의 변경 주체나 시점을 알기 어려워질 수 있다는 겁니다.
...
그래도 여전히 사용하기 쉽고, `GetComponent()`나 `Find()` 등 무거운 함수의 중복 실행을 피할 수 있다는 점 때문에 프로젝트 규모에 따라 충분히 사용할만한 가치가 있습니다.