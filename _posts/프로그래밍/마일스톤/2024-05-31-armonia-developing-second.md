...
제 경우 `Semaphore.WaitForSignal`이 50~70%가량의 지분을 차지하고 있었는데, 이 경우 주로 셰이더를 가벼운 것으로 변경해주는 작업을 권장한다는 글을 보고 [이전에 찾았던 셰이더 파일](https://hynrng.github.io/posts/armonia-developing-first/#%EC%8A%A4%ED%94%84%EB%9D%BC%EC%9D%B4%ED%8A%B8-%EC%85%B0%EC%9D%B4%EB%8D%94)을 보다 가벼운 것으로 교체해줬더니 완벽하진 않지만 60FPS가 안정적으로 유지되고 발열이 매우 줄어들었습니다.

이런 경험은 처음이었는데, 제가 놓치고 있던 부분을 발견해서 만족스럽습니다. 앞으로도 최적화는 주기적으로 신경 써야 할 것 같습니다.
...
6월에 사회복무요원 복무가 시작되기 때문에 하던 일을 잠시 접어두고 훈련소를 가야 합니다. 향후 상황을 아직 잘 모르기 때문에 가능할지는 모르겠지만, 그래도 스트어 등록이 가능한 수준을 목표로 개발을 차근차근 이어나가고 싶습니다.